    ...
    Plugin.prototype = {
        ...
        _init: function () {
            ...
            this.input = $(
				...
				' data-placeholder="' + (this.element.find(":selected").text() || this.element.attr("placeholder") || (op.placeholder != null) ? op.placeholder : this.element.find("option:first").text()) +
				'" ' + (this.element.is('[tabindex]') ? ('tabindex=' + this.element.attr('tabindex')) : '') + '>' +
				(this.element.find(":selected").html() || "") +
				'</span>'
            ...
            if (this.element.find(":selected").length) {
                this._showResetLink();
                this.items.filter('[data-value="' + this.element.find(":selected").val() + '"]').addClass(op.class_selected);
            }
